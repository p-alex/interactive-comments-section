...
const insertReplies = (replies, currentUser) => {
    let result = "";
    replies.forEach((reply) => {
        const { score, user, createdAt, content } = reply;
        result += `<div class="commentBox__container">${createCommentElement(score, user.image.png, user.username, createdAt, content, [], currentUser).innerHTML}</div>`;
    });
    return result;
};
const deleteButton = () => {
